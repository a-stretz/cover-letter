...
from scout.registry import get_recent_postings, get_run_history
...
def scout_run():
    """
    Run the JADE Job Scout pipeline.

    Optional JSON body:
      tier          int   1|2|3 — only poll companies at this tier
      skip_fetch    bool  skip Exa content fetch (dry run)
      skip_score    bool  skip Sonnet scoring (dry run)
      min_score     float minimum score for results returned (default 5.0)
    """
    ...
    try:
        result = run_scout(
            tier_filter=tier,
            skip_fetch=bool(data.get('skip_fetch', False)),
            skip_score=bool(data.get('skip_score', False)),
            min_score_to_return=float(data.get('min_score', 5.0)),
        )
        return jsonify(result)
    except Exception as e:
        import traceback
...
@app.route('/scout/results')
def scout_results():
    """Return recent scored job postings from the registry."""
    limit = min(int(request.args.get('limit', 50)), 200)
    min_score = request.args.get('min_score')
    postings = get_recent_postings(
        limit=limit,
...
@app.route('/scout/history')
def scout_history():
    """Return recent scout run history."""
    limit = min(int(request.args.get('limit', 10)), 50)
    return jsonify(get_run_history(limit=limit))
